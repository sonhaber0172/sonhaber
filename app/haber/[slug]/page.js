...
export default async function HaberDetay({ params }) {
  ...
  const haber = allNews.find(h => h.title === slug || h.title?.substring(0, 50) === slug?.substring(0, 50))
      ...
      <main className="min-h-screen bg-gray-50">
        <header className="bg-red-700 text-white shadow-lg">
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            <Link href="/">
              <h1 className="text-3xl font-bold tracking-tight cursor-pointer">SONHABER</h1>
            </Link>
          </div>
        </header>
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-xl shadow-md p-8">
          ...
          <p className="text-gray-400 text-sm mb-6">{new Date(haber.created_at).toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        ...
        <div className="mt-4">
          <Link href="/" className="text-red-600 hover:text-red-700 font-medium">← Ana sayfaya dön</Link>
        </div>
      </div>
