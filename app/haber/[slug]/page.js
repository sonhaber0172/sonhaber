...
export async function generateMetadata({ params }) {
  ...
  let title = 'Son Dakika Haberleri | HaberSon'
...
export default async function HaberDetay({ params }) {
  ...
  const breadcrumbSchema = haber ? {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Ana Sayfa", "item": "https://sonhaber-rouge.vercel.app" },
      { "@type": "ListItem", "position": 2, "name": haber.category || "Gündem", "item": `https://sonhaber-rouge.vercel.app/?kategori=${encodeURIComponent(haber.category || 'Gündem')}` },
      { "@type": "ListItem", "position": 3, "name": haber.title, "item": haberUrl }
    ]
  } : null
  ...
  const schemaData = haber ? {
    ...
    "author": { "@type": "Organization", "name": "HaberSon", "url": "https://sonhaber-rouge.vercel.app" },
    "publisher": { "@type": "Organization", "name": "HaberSon", "url": "https://sonhaber-rouge.vercel.app" },
    "mainEntityOfPage": { "@type": "WebPage", "@id": haberUrl },
            ...
            <div className="px-6 py-4">
              <Link href="/">
                <div style={{borderLeft: '5px solid #c0392b', paddingLeft: '12px', cursor: 'pointer', display: 'inline-block'}}>
                  <span style={{fontFamily: 'Arial Black, sans-serif', fontSize: '24px', fontWeight: '900', color: '#c0392b'}}>HABER</span>
                  <span style={{fontFamily: 'Arial Black, sans-serif', fontSize: '24px', fontWeight: '900', color: '#111'}}>SON</span>
                </div>
              </Link>
            </div>
          </header>
      ...
      <main className="sayfa-icerik" style={{width: '100%', maxWidth: '1200px', boxShadow: '0 0 30px rgba(0,0,0,0.08)', overflowX: 'hidden'}}>
        ...
        <header className="bg-white border-b border-gray-100">
          <div className="px-4 py-4 flex items-center justify-between">
            <Link href="/">
              <div style={{borderLeft: '5px solid #c0392b', paddingLeft: '10px', cursor: 'pointer'}}>
                <div style={{display: 'flex', alignItems: 'baseline'}}>
                  <span style={{fontFamily: 'Arial Black, sans-serif', fontSize: '24px', fontWeight: '900', color: '#c0392b'}}>HABER</span>
                  <span style={{fontFamily: 'Arial Black, sans-serif', fontSize: '24px', fontWeight: '900', color: '#111'}}>SON</span>
                </div>
                <span style={{fontSize: '9px', color: '#555', letterSpacing: '2px', fontWeight: '700'}}>TÜRKİYE'NİN SESİ</span>
              </div>
            </Link>
          ...
          <div className="px-4 py-1.5 text-xs text-gray-600 border-t border-gray-50">
            {new Date().toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
          </div>
        </header>

        <div className="flex flex-col lg:flex-row">
          <div className="lg:w-2/3 py-4 px-4 border-r border-gray-100" style={{overflowX: 'hidden', maxWidth: '100%'}}>
            {haber.image_url && (
              <img src={haber.image_url} alt={haber.title} className="w-full h-48 md:h-80 object-cover mb-4 rounded-xl" style={{maxWidth: '100%'}} />
            )}
            ...
            <h1 className="text-lg md:text-2xl font-bold text-gray-900 mt-3 mb-2 leading-tight">{haber.title}</h1>
            <p className="text-gray-500 text-xs mb-4 border-b border-gray-100 pb-3">
            ...
            <div className="ilgili-haberler-kutu mt-6 p-4 bg-red-50 rounded-xl border border-red-100">
              ...
              <div className="flex flex-col gap-2">
                {ayniKategori.slice(0, 3).map((h, i) => (
                  <Link key={i} href={`/haber/${encodeURIComponent(h.id)}`} className="text-sm font-semibold hover:text-red-600 transition-colors flex items-start gap-2" style={{color: '#111111'}}>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
              <p className="text-sm font-black text-gray-700 uppercase tracking-widest mb-3 flex items-center gap-2">
                <span style={{display:'inline-block', width:'4px', height:'20px', background:'#c0392b', borderRadius:'2px'}}></span>
                Bu Haberi Paylaş
              </p>
              <div className="flex gap-2 flex-wrap">
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white text-xs font-semibold px-3 py-2 rounded-lg transition-colors">
                ...
                <a href={twitterUrl} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 bg-black hover:bg-gray-800 text-white text-xs font-semibold px-3 py-2 rounded-lg transition-colors">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="white">
            ...
            <div className="mt-6 border-t border-gray-100 pt-4">
              <h2 className="text-sm font-bold uppercase tracking-widest mb-4" style={{color: '#111111'}}>Bunları da Okuyun</h2>
                  ...
                  <Link key={i} href={`/haber/${encodeURIComponent(h.id)}`}>
                    <div className="group cursor-pointer">
                      <div className="overflow-hidden rounded-lg">
                        {h.image_url ? (
                          <img src={h.image_url} alt={h.title} loading="lazy" className="w-full h-20 object-cover group-hover:scale-105 transition-transform duration-300" />
                        ) : (
                          <div className="w-full h-20 bg-gray-100 rounded-lg flex items-center justify-center">
                            <span className="text-gray-300 font-black">HS</span>
                          </div>
                        )}
                      </div>
                      <div className="pt-1">
                        <span className="text-xs text-red-600 font-semibold uppercase tracking-wide">{h.category}</span>
                        <p className="text-xs font-semibold line-clamp-2 mt-0.5 group-hover:text-red-600 transition-colors" style={{color: '#111111'}}>{h.title}</p>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          <div className="lg:w-1/3 px-4 py-6 bg-gray-50">
            <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4">Diğer Haberler</h2>
            <div className="flex flex-col gap-3">
              {sagHaberler.map((h, i) => (
                <Link key={i} href={`/haber/${encodeURIComponent(h.id)}`}>
                  <div className="group cursor-pointer flex gap-3 pb-3 border-b border-gray-200 last:border-0">
                    <div className="overflow-hidden rounded-lg shrink-0" style={{width: '72px', height: '56px'}}>
                      {h.image_url ? (
                        ...
                        <div className="w-full h-full bg-gray-100 rounded-lg flex items-center justify-center">
                          <span className="text-gray-300 font-black text-sm">HS</span>
                        </div>
                      )}
                    </div>
        ...
        <footer className="bg-gray-900 text-white mt-4">
          <div className="px-4 py-8 grid grid-cols-1 gap-6 border-b border-gray-800">
            <div>
              <div style={{borderLeft: '4px solid #c0392b', paddingLeft: '10px', marginBottom: '12px'}}>
                <span style={{fontFamily: 'Arial Black, sans-serif', fontSize: '20px', fontWeight: '900', color: '#fff'}}>HABER</span>
                <span style={{fontFamily: 'Arial Black, sans-serif', fontSize: '20px', fontWeight: '900', color: '#c0392b'}}>SON</span>
              </div>
            ...
            <div>
              <h4 className="text-sm font-bold text-gray-300 uppercase tracking-widest mb-3">Kategoriler</h4>
            ...
            <div>
              <h4 className="text-sm font-bold text-gray-300 uppercase tracking-widest mb-3">Takip Edin</h4>
              <div className="flex flex-col gap-3">
