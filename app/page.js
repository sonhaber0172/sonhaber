  ...
  return (
    <div style={{background: '#ffffff', minHeight: '100vh', display: 'flex', justifyContent: 'center'}}>
      <div style={{display: 'flex', width: '100%', maxWidth: '1200px', justifyContent: 'center'}}>
        ...
        <main style={{width: '100%', maxWidth: '1200px', background: '#ffffff', boxShadow: '0 0 40px rgba(0,0,0,0.15)', flex: 1}}>
        ...
        <div className="hidden xl:flex flex-col gap-4 p-4 w-64 shrink-0 sticky top-4">
          <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
            <div className="bg-red-600 px-4 py-3">
              <h3 className="text-white font-black text-sm">SON HABERLER</h3>
            </div>
              ...
              {allNews.slice(0, 8).map((news, index) => (
                <Link key={index} href={`/haber/${encodeURIComponent(news.id)}`}>
                  <div className="p-3 hover:bg-gray-50 transition-colors">
                    <span className="text-xs text-red-600 font-bold">{news.category}</span>
                    <p className="text-xs font-bold text-gray-800 line-clamp-2 mt-1 leading-snug">{news.title}</p>
                    <p className="text-xs text-gray-400 mt-1">{new Date(news.created_at).toLocaleDateString('tr-TR')}</p>
                  </div>
                </Link>
              ))}
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
            <div className="bg-gray-800 px-4 py-3">
              <h3 className="text-white font-black text-sm">REKLAM VER</h3>
            </div>
            <div className="p-4 text-center">
              <p className="text-gray-500 text-xs mb-3">Bu alana reklam vermek için bizimle iletişime geçin.</p>
              <Link href="/iletisim"
                className="block bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg text-xs transition-colors">
                İletişime Geç
              </Link>
            </div>
          </div>
