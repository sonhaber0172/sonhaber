...
export default async function EkonomiPage({ searchParams }) {
  ...
  const tumHaberler = [...(customNews || []), ...rssNews].filter(n => n.category === 'Ekonomi')
  ...
  const oncekiSayfa = sayfaNo > 1 ? `/ekonomi?sayfa=${sayfaNo - 1}` : null
  const sonrakiSayfa = sayfaNo < toplamSayfa ? `/ekonomi?sayfa=${sayfaNo + 1}` : null
  return <KategoriSayfasi haberler={haberler} kategoriAdi="Ekonomi" sayfaNo={sayfaNo} toplamSayfa={toplamSayfa} oncekiSayfa={oncekiSayfa} sonrakiSayfa={sonrakiSayfa} />
}