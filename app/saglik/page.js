...
export default async function SaglikPage({ searchParams }) {
  ...
  const tumHaberler = [...(customNews || []), ...rssNews].filter(n => n.category === 'Sağlık')
  ...
  const oncekiSayfa = sayfaNo > 1 ? `/saglik?sayfa=${sayfaNo - 1}` : null
  const sonrakiSayfa = sayfaNo < toplamSayfa ? `/saglik?sayfa=${sayfaNo + 1}` : null
  return <KategoriSayfasi haberler={haberler} kategoriAdi="Sağlık" sayfaNo={sayfaNo} toplamSayfa={toplamSayfa} oncekiSayfa={oncekiSayfa} sonrakiSayfa={sonrakiSayfa} />
}